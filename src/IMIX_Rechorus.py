...
class train_thread(threading.Thread):
    ...
    def run(self):
        ...
        # users1, pos_items1, neg_items1 = users[idx], pos_items[idx], neg_items[idx]

        nums = len(users) // 2
        users1, pos_items1, neg_items1 = users[:nums], pos_items[:nums], neg_items[:nums]
        users2, pos_items2, neg_items2 = users[nums:], pos_items[nums:], neg_items[nums:]
        users, pos_items, neg_items = users2, pos_items2, neg_items2
        assert np.sum(np.abs(np.array(users) - np.array(users1))) == 0

        # Lambda = np.abs(np.random.beta(0.5, 0.5, (len(users), 1)) - 0.5) + 0.5
        Lambda = np.random.beta(0.5, 0.5, (len(users), 1))
        self.data = sess.run(
            ...
            feed_dict={model.users: users, model.pos_items: pos_items,
                       ...
                       model.users1: users1, model.pos_items1: pos_items1,
                       model.neg_items1: neg_items1, model.Lambda: Lambda
                       })
    ...
    def run(self):
        ...
        self.data = sess.run([self.model.loss, self.model.mf_loss, self.model.emb_loss],
                             feed_dict={model.users: users, model.pos_items: pos_items,
                                        ...
                                        model.node_dropout: eval(args.node_dropout),
                                        model.mess_dropout: eval(args.mess_dropout),
                                        model.users1: users, model.pos_items1: pos_items,
                                        model.neg_items1: neg_items, model.Lambda: Lambda})
